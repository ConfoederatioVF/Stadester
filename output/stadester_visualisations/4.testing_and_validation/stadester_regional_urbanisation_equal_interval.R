...
for (current_region in regions) {
  ...
  tryCatch({
    ...
    p <- ggplot(plot_df, aes(x = year_factor)) +
      ...
      geom_line(aes(y = rural_population, color = "Rural Population", group = 1), size = 1, na.rm = TRUE) +
      geom_line(aes(y = urban_share_scaled, color = "Urban Share", group = 1), linetype = "dashed", size = 1, na.rm = TRUE) +
      geom_line(aes(y = rural_share_scaled, color = "Rural Share", group = 1), linetype = "dashed", size = 1, na.rm = TRUE) +
